...
export const App = () => {
  ...
  const handleUnlockSubmit = () => {
    ...
    if (isValidKey(key)) {
       ...
       setUnlockKeyInput('');
       alert("Успешно отключихте Gemini 3.0 Pro!");
      ...
      <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in">
        <div className="bg-white dark:bg-zinc-900 w-full max-w-sm p-8 rounded-3xl border border-indigo-500/20 shadow-2xl space-y-6 relative animate-in zoom-in-95 duration-300">
          ...
          <div className="flex flex-col items-center gap-4 text-center">
            <div className="p-4 bg-indigo-500/10 rounded-full text-indigo-500 shadow-lg shadow-indigo-500/20"><Lock size={32}/></div>
            <div>
              <h2 className="text-xl font-bold">Отключи Pro</h2>
              <p className="text-sm text-gray-500 mt-1">Въведете вашия продуктов ключ.</p>
            </div>
          </div>
          <input
            type="text"
            ...
            placeholder="Въведете Ключ (напр. UCH-XXXX-YYYY)"
            ...
            autoFocus
          />
          <Button onClick={handleUnlockSubmit} className="w-full py-4 text-base shadow-indigo-500/30">Отключи</Button>
        </div>
      </div>
    );
  };
              ...
              <div className="p-10 overflow-y-auto custom-scrollbar space-y-12">
                 ...
                 <section>
                    ...
                    <div className="space-y-6">
                        {/* Theme Color */}
                        <div className="p-5 bg-white dark:bg-white/5 rounded-3xl border border-indigo-500/20 flex items-center justify-between relative group/color">
                            <div className="flex flex-col gap-1">
                                <span className="font-bold text-lg flex items-center gap-2">
                                    Основен Цвят
                                    {!isProUnlocked && <div className="bg-amber-500/10 text-amber-500 text-[10px] px-2 py-0.5 rounded-full border border-amber-500/20 font-bold flex items-center gap-1"><Lock size={10}/> PRO</div>}
                                </span>
