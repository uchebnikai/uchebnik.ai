...
export const SYSTEM_PROMPTS = {
  ...
  1. Ако потребителят поиска промяна (напр. "добави още 2 въпроса", "промени въпрос 3", "направи го по-труден"), ТРЯБВА да вземеш предвид съществуващия тест от историята на чата.
  2. Върни ЦЕЛИЯ обновен тест (всички стари въпроси + новите/променените) като един валиден JSON обект. НЕ връщай само новите части.

  ВАЖНО ЗА ФОРМАТИРАНЕТО:
  1. Върни резултата в СТРИКТЕН JSON формат.
  2. НЕ използвай Markdown форматиране (като **bold** или *italic*) вътре в текстовете на въпросите.
  3. НЕ използвай LaTeX код (като $x^2$, \\frac, \\sqrt). 
  4. Вместо LaTeX, използвай СТАНДАРТНИ UNICODE СИМВОЛИ за математика.
     Примери:
     - Вместо x^2 използвай x²
     - Вместо x^3 използвай x³
     - Вместо sqrt(x) използвай √x
     - Вместо alpha, pi използвай α, π
     - Използвай символите: +, -, ×, ÷, =, ≠, ≈, °, ∠, ∆
  5. Целта е текстът да е чист и четим директно.
  
  Формат на JSON (schema):
  {
    "title": "Заглавие на теста",
    "subject": "Предмет (напр. Математика, История)",
    "grade": "Клас (ако е приложимо)",
    "questions": [
       {
         "id": 1,
         "question": "Текст на въпроса (Plain Text + Unicode Math)",
  ...
  Върни само JSON обекта, без допълнителен текст.`,
