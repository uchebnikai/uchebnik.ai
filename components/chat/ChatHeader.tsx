      ...
      <header className={`sticky top-0 lg:top-4 mx-0 lg:mx-8 z-30 h-16 lg:h-18 
        ...
        border-b lg:border lg:shadow-sm lg:rounded-3xl flex items-center justify-between px-4 lg:px-6 transition-all duration-300 pt-safe`}>
         ...
         <div className="flex items-center gap-3 lg:gap-5 overflow-hidden flex-1 min-w-0">
            {isGuest ? (
                <button onClick={() => setActiveSubject(null)} className="p-2 -ml-2 rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 text-gray-500 dark:text-gray-400 shrink-0 transition-colors">
                    <ArrowLeft size={24}/>
                </button>
            ) : (
            ...
            <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-xl lg:rounded-2xl ${activeSubject?.color} flex items-center justify-center text-white shadow-lg shadow-indigo-500/20 shrink-0`}>
                <DynamicIcon name={activeSubject?.icon || 'Book'} className="w-5 h-5 lg:w-6 lg:h-6"/>
            </div>
            
            <div className="overflow-hidden min-w-0 flex-1">
               <h2 className="font-bold text-zinc-900 dark:text-white leading-none text-base lg:text-lg tracking-tight truncate pr-2">
                   {isGuest ? "Demo Chat" : activeSubject?.name}
               </h2>
               {activeSubject?.id === SubjectId.GENERAL ? (
                   <div className="text-xs text-gray-500 font-medium mt-1">
                       {isGuest ? "Влезте в профила си, за да запазите чата." : "Chat Assistant"}
                   </div>
               ) : (
         ...
         <div className="flex items-center gap-1.5 lg:gap-3 shrink-0 ml-2">
             {!isGuest ? (
                 ...
                 <div className="flex items-center gap-2">
                     <span className="hidden sm:inline text-xs font-bold text-indigo-400 bg-indigo-500/10 px-3 py-1.5 rounded-lg border border-indigo-500/20">Demo Mode</span>
                 </div>
             )}
         </div>
      </header>
