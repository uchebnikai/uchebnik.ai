...
export const AdSenseContainer = ({ userPlan }: AdSenseContainerProps) => {
  ...
  useEffect(() => {
    ...
    // Reset adLoaded when visibility or mode changes to re-trigger push if needed
    setAdLoaded(false);

    const timer = setTimeout(() => {
      try {
        if (typeof window !== 'undefined' && (window as any).adsbygoogle) {
          // Check if there are any uninitialized ads in the DOM
    ...
    }, 800); // Slightly longer delay to ensure layout is settled

    return () => clearTimeout(timer);
  }, [userPlan, isVisible, isMobile]);
        ...
        <div className="fixed bottom-4 right-4 z-[45] w-[300px] animate-in slide-in-from-right duration-700">
          <div className="glass-panel p-2 pb-1 overflow-hidden shadow-2xl border-indigo-500/20 bg-white/40 dark:bg-black/40 backdrop-blur-xl">
            ...
            <div className="bg-white/5 rounded-2xl overflow-hidden min-h-[250px] flex items-center justify-center text-zinc-500 text-xs italic border border-white/5">
              ...
              {!adLoaded && <span className="absolute animate-pulse">Loading Sponsor...</span>}
            </div>
          </div>
        </div>
      )}
      ...
      {isMobile && (
        <div className="w-full px-2 mt-2 z-20 animate-in slide-in-from-top duration-500">
          <div className="glass-panel p-1.5 overflow-hidden border-indigo-500/10 bg-white/20 dark:bg-black/20 backdrop-blur-md">
              ...
              <div className="bg-white/5 rounded-xl overflow-hidden min-h-[60px] flex items-center justify-center text-[10px] text-zinc-500 italic border border-white/5">
                  ...
                  <ins className="adsbygoogle"
                       ...
                       data-full-width-responsive="true"></ins>
                  {!adLoaded && <span className="absolute animate-pulse">Sponsor Space</span>}
              </div>
          </div>
        </div>
      )}
